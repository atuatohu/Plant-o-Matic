...
void DHT11::run(DHT11* dht11){
	int prev_h1, prev_h2;
	prev_h1 = dht11->dht11_dat[0];
	prev_h2 = dht11->dht11_dat[1]; 
	int prev_t1, prev_t2;
	prev_t1 = dht11->dht11_dat[2];
	prev_t2 = dht11->dht11_dat[3];
	dht11->running = 1;
	while(dht11->running){
		if(dht11->read_dht_data()){
			prev_h1 = dht11->dht11_dat[0];
			...
			dht11->dht11_dat[0] = prev_h1;
			dht11->dht11_dat[1] = prev_h2;
			dht11->dht11_dat[2] = prev_t1;
			dht11->dht11_dat[3] = prev_t2;
			if(dht11->dht11callback){
				dht11->dht11callback->hasSample(dht11->dht11_dat);
			}
		}
